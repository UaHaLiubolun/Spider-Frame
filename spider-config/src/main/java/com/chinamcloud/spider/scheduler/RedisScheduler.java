...
public class RedisScheduler extends DuplicateRemovedScheduler implements MonitorableScheduler,DuplicateRemover {
    ...
    private static final String SET_HELP_PREFIX = "set_help_";
    ...
    public boolean isDuplicate(Request request, Task task) {
        Jedis jedis = pool.getResource();
        try {
            return jedis.sadd(getSetKey(request, task), request.getUrl()) == 0;
        } finally {
            jedis.close();
        }
    }
    ...
    protected String getSetHelpKey(Task task) {return SET_HELP_PREFIX + task.getUUID();}
    ...
    private String getSetKey(Request request, Task task) {
        if (request.isExtractLinks()) {
            return getSetHelpKey(task);
        } else {
            return getSetKey(task);
        }
    }
}
